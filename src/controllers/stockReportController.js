...
const buildStockReport = async ({ branchId, from, to, top = 10, limit = 200 }) => {
  ...
  const lowStock = await Ingredient.find({ $expr: { $lte: ['$currentStock', '$reorderLevel'] }, ...(branchId ? { branchId } : {}) })
...
const stockReport = async (req, res) => {
  try {
    ...
    const data = await buildStockReport({ branchId: req.branchId, from, to, top, limit });
    return res.json(data);
  } catch (error) {
    return res.status(500).json({ message: 'Stock report failed', error: error.message });
  }
};
module.exports = { stockReport, buildStockReport };
